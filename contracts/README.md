...
Compile from the repository root with `npm run contracts:compile`. Deploy with a multisig as `initialGovernor`, audit each concrete checker/adapter, approve adapters through `setAdapterApproval`, and publish the deployed address as `SHIELD_AUTOMATION_CONTRACT_ADDRESS`.
