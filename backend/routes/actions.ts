...
function keyCheck(payload: ParsedActionPayload, mode: ExecutionMode, _keyStatus: ReturnType<typeof sodexTrader.getKeyStatus>) {
  ...
  const readiness = sodexTrader.getExecutionReadiness(payload.network);
  return { passed: readiness.ready, message: readiness.message };
}
...
router.post('/prepare', requireOperator, asyncHandler(async (req: Request, res: Response) => {
  ...
  const liveExecution = payload.action !== 'QUEUE_ACTION' && ['testnet', 'mainnet_canary'].includes(executionMode);
  const readiness = sodexTrader.getExecutionReadiness(payload.network);
  if (liveExecution && !readiness.ready) {
    return res.status(503).json({ error: readiness.message, code: 'SODEX_EXECUTION_NOT_READY' });
  }

  try {
    await hydrateExecutionContext(payload, liveExecution ? readiness.accountAddress! : session.address);
  } catch (error) {
    return res.status(422).json({ error: error instanceof Error ? error.message : 'Could not verify execution context.' });
  }

  const simulation = buildSimulation(payload, liveExecution ? readiness.accountAddress! : session.address);
  ...
  return res.json({
    ...
    executionAuthorization: liveExecution ? 'managed_registered_api_key' : 'none'
  });
}));

router.post('/simulate', requireOperator, asyncHandler(async (req: Request, res: Response) => {
  let payload: ParsedActionPayload;
  try {
    payload = parsePayload(req.body);
  } catch (error) {
  ...
  payload.network = session.network;
  const executionMode = String(process.env.EXECUTION_MODE || 'dry_run').toLowerCase();
  const liveExecution = ['testnet', 'mainnet_canary'].includes(executionMode);
  const executionNetwork: Network = executionMode === 'mainnet_canary' ? 'mainnet' : 'testnet';
  if (liveExecution && payload.network !== executionNetwork) {
  ...
  const executionAccount = liveExecution ? String(process.env.SODEX_ACCOUNT_ADDRESS || '').trim() : session.address;
  try {
    await hydrateExecutionContext(payload, executionAccount);
  } catch (error) {
    return res.status(422).json({ error: error instanceof Error ? error.message : 'Could not verify execution context.' });
  }
  const simulation = buildSimulation(payload, liveExecution ? executionAccount : session.address);
...
router.post('/confirm-wallet', requireOperator, asyncHandler(async (req: Request, res: Response) => {
  const session = walletAuth.getWalletSession(req);
  if (!session) return res.status(401).json({ error: 'Connect and sign in with your SoDEX wallet.' });

  return res.status(409).json({
    error: 'Connected-wallet trading is disabled. Submit through /api/actions/confirm so the registered managed API key signs the action.',
    code: 'CONNECTED_WALLET_EXECUTION_UNSUPPORTED'
  });
}));
...
async function executePayload(req: Request, res: Response) {
  ...
  payload.wallet = session.address.toLowerCase();
  payload.network = session.network;
  const configuredMode = String(process.env.EXECUTION_MODE || 'dry_run').toLowerCase();
  const liveExecution = payload.action !== 'QUEUE_ACTION' && ['testnet', 'mainnet_canary'].includes(configuredMode);
  const readiness = sodexTrader.getExecutionReadiness(payload.network);
  if (liveExecution && !readiness.ready) {
    return res.status(503).json({ error: readiness.message, code: 'SODEX_EXECUTION_NOT_READY' });
  }
  ...
  try {
    ...
    const recent = await executionLedger.findRecentExecution({
      requestedBy: liveExecution ? null : session.address.toLowerCase(),
...
router.post('/confirm', requireOperator, asyncHandler(executePayload));
router.post('/', requireOperator, asyncHandler(executePayload));
