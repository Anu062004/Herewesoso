...
router.get('/', async (_req: Request, res: Response) => {
  ...
  const { data, error } = await safeSelect('trade_memos', (query: any) =>
    query.or(`wallet_address.is.null,wallet_address.eq.${wallet}`).order('created_at', { ascending: false }).limit(5)
  );
  if (isProduction()) {
    if (error) return res.status(503).json({ error: 'Memo storage is temporarily unavailable.' });
