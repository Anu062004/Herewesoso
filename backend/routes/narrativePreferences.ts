...
router.post('/', async (req: Request, res: Response) => {
  ...
  const stages = Array.isArray(body.stages)
    ...
    : defaults.stages;
  const minConfidence = Math.max(0, Math.min(100, Number(body.minConfidence ?? defaults.minConfidence)));
  const maxCrowding = Math.max(0, Math.min(100, Number(body.maxCrowding ?? defaults.maxCrowding)));
